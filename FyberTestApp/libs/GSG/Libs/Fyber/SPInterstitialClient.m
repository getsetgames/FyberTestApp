...
static const NSInteger SPInterstitialClientInvalidStateErrorCode = -9;
static const NSInteger SPInterstitialClientConnectionErrorCode = -10;
...
static NSString *const SPInterstitialClientOfferDataTrackingParamsKey = @"SPInterstitialClientTrackingParamsKey";
...
{
    [self checkInterstitialAvailableForPlacementId:nil];
}
...
- (void)checkInterstitialAvailableForPlacementId:(NSString *)placementId
{
    ...
    void (^requestCompletionHandler)(NSURLResponse *, NSData *, NSError *) = ^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        SPInterstitialResponse *interstitialResponse = [SPInterstitialResponse responseWithURLResponse:response data:data connectionError:connectionError];

        if (!interstitialResponse.isSuccessResponse) {
            self.state = SPInterstitialClientReadyToCheckOffersState;
            [self failWithErrorDescription:@"An error occurred while requesting interstitial offers" underlyingError:interstitialResponse.error];
            return;
        }
        ...
        if (selectedOffer) {
            ...
            // we have to send a global fill without the offer id
            ...
            // we have to send a global no_fill without the offer id
    ...
    [SPURLRequest requestWithUserDataForURL:requestForOffers.URL shouldUpdateLocation:YES completionBlock:^(NSURLRequest *urlRequest) {
        SPLogDebug(@"Requesting interstitial offers: %@", [urlRequest.URL SPPrettyDescription]);
...
- (BOOL)canShowInterstitialForOffer:(SPInterstitialOffer *)offer
{
    ...
    dict[SPInterstitialClientOfferDataTrackingParamsKey] = offer.trackingParams;
    ...
    return offer.isOrientationSupported && [adapter canShowInterstitial];
}
...
- (void)showInterstitialFromViewController:(UIViewController *)parentViewController
{
    ...
    SPLogDebug(@"Showing interstitial from network: %@", self.selectedOffer.networkName);
...
- (void)adapter:(id<SPInterstitialNetworkAdapter>)adapter didFailWithError:(NSError *)error
{
    ...
    NSDictionary *trackingParams = adapter.offerData[SPInterstitialClientOfferDataTrackingParamsKey];

    [self fireNotification:SPInterstitialEventTypeError network:adapter.networkName adId:adId requestId:request_id trackingParams:trackingParams];
          ...
          trackingParams:(NSDictionary *)trackingParams
{
    SPInterstitialEvent *event =
            [[SPInterstitialEvent alloc] initWithEventType:eventType
                                                   network:networkName
                                                      adId:adId
                                                 requestId:requestId
...
{
    [self fireNotification:eventType network:offer.networkName adId:offer.adId requestId:self.lastRequestID trackingParams:offer.trackingParams];
}
