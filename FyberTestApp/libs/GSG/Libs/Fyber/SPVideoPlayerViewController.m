...
- (void)viewDidLoad
{
    ...
    self.stallTimer = [NSTimer scheduledTimerWithTimeInterval:SPVideoTimeout target:weakSelf selector:@selector(videoStalledAtStartup) userInfo:nil repeats:NO];
...
- (void)viewDidLayoutSubviews
{
    ...
    self.closeButton.frame = [self.closeButton frameForCloseButtonInVideoPlayerInFrame:self.view.bounds];
}
...
- (void)trackProgress
{
    ...
    if (self.player.playbackState == MPMoviePlaybackStatePlaying) {
        if ([self.delegate respondsToSelector:@selector(timeUpdate:duration:)]) {
...
{
    SPLogDebug(@"Movie playback changed to state %@", @(self.player.playbackState));
    if (self.player.playbackState == MPMoviePlaybackStatePlaying) {
        [self.loadingIndicator dismiss];
        ...
        if (!self.hasVideoPlayed) {
            ...
            if ([self.delegate respondsToSelector:@selector(videoPlaybackStartedWithDuration:)]) {
...
{
    SPLogDebug(@"Video duration is: %@", @(self.player.duration));
    __weak id weakSelf = self;
    SPLogDebug(@"Scheduling playbackTimer timer");
    self.playbackTimer = [NSTimer scheduledTimerWithTimeInterval:SPUpdateInterval target:weakSelf selector:@selector(trackProgress) userInfo:nil repeats:YES];
...
- (void)openClickThroughBrowser
{
    ...
    if ([self.delegate respondsToSelector:@selector(browserWillOpen)]) {
...
{
    CGRect closeButtonFrame = [self.closeButton frameForCloseButtonInVideoPlayerInFrame:self.view.bounds];
    self.closeButton.center = CGPointMake(CGRectGetMidX(closeButtonFrame), CGRectGetMidY(closeButtonFrame));
    self.countdownView.frame = [self frameForCountDownView];

    if (UIInterfaceOrientationIsPortrait(toInterfaceOrientation)) {
        self.countdownView.transform = CGAffineTransformMakeRotation((CGFloat) M_PI_2);
    } else {
...
{
    if (UIInterfaceOrientationIsPortrait(toInterfaceOrientation)) {
        self.player.view.transform = CGAffineTransformMakeRotation((CGFloat) M_PI_2);
...
- (void)videoPlaybackFinishedWithAbort:(BOOL)videoWasAborted
{
    ...
    [self.player stop];
    [self.loadingIndicator dismiss];
    if ([self.delegate respondsToSelector:@selector(videoPlaybackEnded:)]) {
...
{
    SPLogDebug(@"Invalidating timers");
    if([self.playbackTimer isValid]) {
        [self.playbackTimer invalidate];
    }
    if([self.stallTimer isValid]) {
        [self.stallTimer invalidate];
    }
}
