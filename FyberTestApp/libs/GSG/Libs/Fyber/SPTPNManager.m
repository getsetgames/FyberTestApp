...
- (SPNetworkCompletionBlock) networkCompletionBlock {
    return ^void(NSArray *networks) {
        // Adds Fyber Network to the list
        NSMutableArray *mutableNetworks = [NSMutableArray arrayWithArray:networks];
        [mutableNetworks addObject:@{
                                     SPNetworkName: @"Fyber",
                                     SPNetworkParameters: @{}
                                     }];
        [mutableNetworks enumerateObjectsUsingBlock:^(id providerData, NSUInteger idx, BOOL *stop) {
