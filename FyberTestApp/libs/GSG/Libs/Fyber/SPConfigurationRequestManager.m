...
static NSString *const SPAdaptersFileName = @"SPAdapters.json";
...
+ (id)json:(NSData *)data error:(NSError **)error
{
    ...
    if (!config[SPNetworks]) {
        NSString *errorDescription = [NSString stringWithFormat:@"Invalid JSON. Expected key '%@' in server side configuration response got %@", SPNetworks, config];
